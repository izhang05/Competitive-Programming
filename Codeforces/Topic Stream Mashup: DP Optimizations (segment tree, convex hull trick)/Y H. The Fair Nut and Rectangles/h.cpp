...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
#ifdef DEBUG
        cout << i << " " << -cht.eval(a[i].first.second) << " " << a[i].first.first * a[i].first.second << " " << -a[i].second << "\n";
#endif
        dp[i] = cht.eval(a[i].first.second) + a[i].first.first * a[i].first.second - a[i].second;
        cht.insert_line(-a[i].first.first, dp[i]);
        sol = max(sol, dp[i]);
    }
#ifdef DEBUG
    for (int i = 0; i < n; ++i) {
