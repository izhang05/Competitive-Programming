...
int main() {
    ...
    sort(a.begin(), a.end());
    modnum<mod> number = 1, sum = 1, prod = 1;
    long long num_min_1 = 1;
    for (int i = 0; i < n; ++i) {
        long long c = (int) a[i].second;
        prod = pow(prod, c + 1) * pow(pow(modnum<mod>{a[i].first}, (c * (c + 1)) / 2), num_min_1);
        number *= a[i].second + 1;
        num_min_1 *= a[i].second + 1;
        num_min_1 %= mod - 1;
        modnum<mod> b = a[i].first;
        sum += sum * ((b * (pow(b, c) - 1)) / (b - 1));
    }
