...
vector<long long> vacation(int n, int q, vector<vector<ll>> trees, const vector<vector<ll>>& queries) {
    ...
    auto merge = [](const array<ll, 4> &q, const array<ll, 4> &p) {
        ...
        array<ll, 4> res =
                {(((q[A] * binexp(p[A], q[B])) % MOD) * binexp(p[P], q[X])) % MOD,
                 (q[B] * p[B]) % (MOD - 1),
                 (q[P] * p[P]) % (MOD),
                 (p[X] * q[B] + q[X]) % (MOD - 1)};
