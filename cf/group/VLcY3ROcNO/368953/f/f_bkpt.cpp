...
modnum<mod> dp[MAXN][LOG][MAXS];
modnum<mod> tot[MAXN][LOG][MAXS];
...
void dfs(int c, int l) {
    ...
    for (auto [i, cost] : g[c]) {
        ...
        for (int j = 0; j < LOG - 1; j++) {
            ...
            for (int x = 0; x <= s; x++) {
                //                if (!tot[c][j][x]) continue;
                for (int y = 0; y + cost + x <= s; y++) {

                    modnum<mod> cv = dp[i][j][y] * tot[c][j][x];
                    dp[c][j + 1][x + y + cost] += cv;
                }
            }

            for (int x = 0; x + cost <= s; x++) {
                tot[c][j][x + cost] += dp[i][j][x];
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < LOG; i++) {
        for (int j = 0; j <= s; j++) {
            //            if (dp[1][i][j]) cout << i << " " << j << endl;
            res += dp[1][i][j];
        }
    }

    // res *= binexp(binexp(2, n), MOD - 2);
    // res %= MOD;
    cout << res / pow(modnum<mod>(2), n) << "\n";
}