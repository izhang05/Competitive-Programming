...
const int inf = 0x3f3f3f3f, mod = 1e9 + 7, maxn = 13;
const long long INFL = 0x3f3f3f3f3f3f3f3f;

void solve() {
    long long n, k;
    cin >> n >> k;
    vector<modnum<mod>> dp(n + 6); // first n values = a, [n] = q*i, [n + 1] = q, [n + 2] = i^2*r, [n + 3] = 2*r*i, [n + 4] = r, [n + 5] = p
    for (int i = 0; i < n; ++i) {
        cin >> dp[n - i - 1];
    }
    vector<modnum<mod>> c(n);
    for (int i = 0; i < n; ++i) {
        cin >> c[i];
    }
    modnum<mod> p, q, r;
    ...
    dp[n] = q * n, dp[n + 1] = q;
    dp[n + 2] = n * n * r, dp[n + 3] = 2 * n * r, dp[n + 4] = r;
    dp[n + 5] = p;
    matrix<modnum<mod>, maxn + 6> mat;
    for (int i = 0; i < n; ++i) {
        mat.d[i][0] = c[i];
        if (i) {
            mat.d[i - 1][i] = 1;
    ...
    mat.d[n][0] = mat.d[n + 2][0] = mat.d[n + 5][0] = 1;
    mat.d[n][n] = mat.d[n + 1][n] = 1;
    mat.d[n + 1][n + 1] = 1;
    mat.d[n + 2][n + 2] = mat.d[n + 3][n + 2] = mat.d[n + 4][n + 2] = 1;
    mat.d[n + 3][n + 3] = 1, mat.d[n + 4][n + 3] = 2;
    mat.d[n + 4][n + 4] = 1;
    mat.d[n + 5][n + 5] = 1;

    mat ^= k - n + 1;

    vector<modnum<mod>> sol(n + 6);
    for (int j = 0; j < n + 6; ++j) {
        for (int l = 0; l < n + 6; ++l) {
            sol[j] += dp[l] * mat.d[l][j];
        }
    }
    ...
    //            new_dp[j] = dp[j - 1];
    //        }
    //        new_dp[0] += dp[n] + dp[n + 2] + dp[n + 5];
    //
    //        new_dp[n] += dp[n] + dp[n + 1];
    //        new_dp[n + 1] += dp[n + 1];
    //        new_dp[n + 2] += dp[n + 2] + dp[n + 3] + dp[n + 4];
    //        new_dp[n + 3] += dp[n + 3] + 2 * dp[n + 4];
    //        new_dp[n + 4] += dp[n + 4];
    //        new_dp[n + 5] += dp[n + 5];
    //        swap(dp, new_dp);
    //    }
