...
const int inf = 0x3f3f3f3f, mod = 1e9 + 7, maxt = 1e5 + 5;
const long long INFL = 0x3f3f3f3f3f3f3f3f;

modnum<mod> dp[maxt][2];
...
void test_case() {
    ...
    for (int i = 0; i < t - 1; ++i) {
        //        dbg() << i << " " << dp[i][0] << " " << dp[i][1];
        dp[i + 1][0] += dp[i][0] * (1) / m;                       // keep password
        dp[i + 1][0] += dp[i][0] * (m - 1) / m * (k - 1) / n;     // change password
        dp[i + 1][1] += dp[i][0] * (m - 1) / m * (n + 1 - k) / n; // change password

        dp[i + 1][1] += dp[i][1] * 1 / m;
        dp[i + 1][1] += dp[i][1] * (m - 1) / m * (n - k) / n;
        dp[i + 1][0] += dp[i][1] * (m - 1) / m * (k) / n;
    }
    cout << dp[t - 1][0] << "\n";
}
