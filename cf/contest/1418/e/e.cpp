...
int main() {
    ...
    cin >> n >> m;
    vector<int> a(n);
    modnum<mod> sum_big = 0, small = 0, big = n, sum_small = 0;
    for (int i = 0; i < n; ++i) {
        cin >> a[i];
        sum_big += a[i];
    }
    ...
    for (int i = 0; i < m; ++i) {
        while (j < n && a[j] < b[i].first.second) {
            ++small, --big, sum_small += a[j], sum_big -= a[j];
            ++j;
        }
        if (b[i].first.first > (int) big) {
            sol[b[i].second] = 0;
        } else {
            sol[b[i].second] = sum_big * (1 - modnum<mod>{b[i].first.first} / big) + sum_small * (1 - modnum<mod>{b[i].first.first} / (big + 1));
        }
    }
    for (int i = 0; i < m; ++i) {
        cout << sol[i] << "\n";
