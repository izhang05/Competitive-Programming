...
void test_case() {
    int n, k;
    cin >> n >> k;
    vector<vector<modnum<mod>>> dp(n / k + 1), old_dp(n / k + 1), sum(n / k + 1), old_sum(n / k + 1);
    for (int i = 0; i < n / k + 1; ++i) {
        dp[i].resize(k + 1);
        old_dp[i].resize(k + 1);
        sum[i].resize(k + 1);
        old_sum[i].resize(k + 1);
    }

    old_dp[0][0] = old_sum[0][0] = 1;
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= n / k; ++j) {
            dp[j].clear();
            sum[j].clear();
        ...
        for (int j = 0; j <= n / k; ++j) {
            if (j > 0) {
                dp[j][0] = old_dp[j - 1][k - 1];
            }
            for (int l = 1; l < k; ++l) {
                dp[j][l] = old_sum[j][l] + old_dp[j][l - 1] * (k - l + 1);
            }
            sum[j][k - 1] = dp[j][k - 1];
            for (int l = k - 2; l >= 0; --l) {
                sum[j][l] = sum[j][l + 1] + dp[j][l];
            }
        }
        swap(dp, old_dp);
        swap(sum, old_sum);
