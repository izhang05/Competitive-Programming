...
void test_case() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= i; ++j) {
            ...
            long double take = (actual[i - 1][j] - actual[i - 1][j - 1]) / 2;
            if (take > k) {
                dp[i][j] = dp[i - 1][j - 1] + k;
                actual[i][j] = actual[i - 1][j - 1] + k;
            } else {
                dp[i][j] = dp[i - 1][j - 1] + (dp[i - 1][j] - dp[i - 1][j - 1]) / 2;
                actual[i][j] = actual[i - 1][j - 1] + take;
            }
        }
    }
    cout << dp[n][m] << "\n";
