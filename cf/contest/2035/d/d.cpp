...
void test_case() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        while (!st.empty() && (p >= 30 || st.top().first <= a[i] * cur_l)) {
            sol -= (pow(modnum<mod>{2}, st.top().second) - 1) * st.top().first;
            cur *= pow(modnum<mod>{2}, st.top().second);
            cur_l *= (int) pow(modnum<mod>{2}, st.top().second);
            p += st.top().second;
            st.pop();
        }
        sol += a[i] * cur;
