...
void test_case() {
    ...
    for (int i = 0; i < diff; ++i) {
        modnum<mod> cur = pow(modnum<mod>(k - 2), i) * nCr(diff, i);
        if ((diff - i) % 2 == 1) {
            cur *= pow(modnum<mod>(2), diff - i - 1);
        } else {
            cur *= (pow(modnum<mod>(2), diff - i) - nCr(diff - i, (diff - i) / 2)) / 2;
        }
        sol += cur;
    }
    sol *= pow(modnum<mod>(k), n - diff);
    cout << sol << "\n";
}
