...
vector<int> adj[maxn];
modnum<mod> dp[maxn][2][2]; // dp[node][leaf?][colored?]
modnum<mod> dp2[maxn];
...
void dfs(int c, int p) {
    ...
    for (auto &i : adj[c]) {
        if (i != p) {
            dfs(i, c);
            modnum<mod> sum, sum1 = dp2[i], sum2;
            for (int j = 0; j < 2; ++j) {
                for (int k = 0; k < 2; ++k) {
                    sum += dp[i][j][k];
                }
            ...
            dp2[c] *= sum1;
            dp[c][0][0] = dp[c][0][0] * sum + dp[c][0][0] * sum1 + dp[c][1][0] * sum;
            dp[c][0][1] = dp[c][0][1] * sum2 + dp[c][0][1] * sum1 + dp[c][1][1] * sum2;
            dp[c][1][0] *= sum1;
            dp[c][1][1] *= sum1;
        }
    }
...
void test_case() {
    ...
    dfs(0, -1);
    cout << dp[0][0][0] + dp[0][0][1] + dp2[0] - 1 << "\n";
}
