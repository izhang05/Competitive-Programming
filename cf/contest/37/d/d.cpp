...
modnum<mod> dp[maxn][maxn * maxn], nCr[maxn * maxn][maxn];
...
void test_case() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j < maxn * maxn; ++j) {
            for (int k = max(0, a[i - 1] - j); k <= b[i - 1] && k + j < maxn * maxn; ++k) {
                dp[i][j] += dp[i - 1][j + k - a[i - 1]] * nCr[j + k][k] * nCr[sum][a[i - 1]];
            }
        }
        sum -= a[i - 1];
    }
