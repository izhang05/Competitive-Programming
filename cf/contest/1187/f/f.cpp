...
void test_case() {
    ...
    modnum<mod> sol;
    vector<modnum<mod>> e(n);
    modnum<mod> sum = 0;
    for (int i = 1; i < n; ++i) {
        modnum<mod> inter = max(0, min(r[i], r[i - 1]) - max(l[i], l[i - 1]));
        modnum<mod> denom = (modnum<mod>(r[i] - l[i]) * (r[i - 1] - l[i - 1]));
        e[i] = 1 - inter / denom;
        sum += e[i];
    }
    modnum<mod> pre = 0;
    for (int i = 1; i < n; ++i) {
        modnum<mod> cur_sum = sum - e[i] - e[i - 1];
        if (i + 1 < n) {
            cur_sum -= e[i + 1];
        }
        sol += e[i] * cur_sum + e[i] + pre;
        if (i + 1 < n) {
            modnum<mod> inter = max(0, min({r[i + 1], r[i], r[i - 1]}) - max({l[i + 1], l[i], l[i - 1]}));
            modnum<mod> denom = (modnum<mod>(r[i + 1] - l[i + 1]) * (r[i] - l[i]) * (r[i - 1] - l[i - 1]));
            sol += 1 - ((1 - e[i]) + (1 - e[i + 1]) - inter / denom);
            pre = 1 - ((1 - e[i]) + (1 - e[i + 1]) - inter / denom);
        }
    }
    cout << sol << "\n";
}
