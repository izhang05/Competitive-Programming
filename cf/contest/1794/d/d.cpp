...
modnum<mod> f[maxn], invf[maxn];
modnum<mod> dp[4050][4050];
bool prime[maxn];
...
void test_case() {
    ...
    dp[0][0] = 1;
    int m = int(primes.size()), cnt = 1;
    for (auto &i : primes) {
        for (int j = 0; j <= cnt; ++j) {
            dp[cnt][j] = dp[cnt - 1][j] * invf[i.second];
            if (j) {
                dp[cnt][j] += dp[cnt - 1][j - 1] * invf[i.second - 1];
            }
        }
        ++cnt;
    }
