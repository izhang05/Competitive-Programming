...
modnum<mod> dp[maxn][maxn]; // dp[col][cnt]
modnum<mod> nCr[maxn][maxn];
...
void test_case() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 0; j <= n; ++j) {
            dp[i][j] = dp[i - 1][j] * (pow(modnum<mod>(m), j) * pow(modnum<mod>(m - 1), n - j) - pow(modnum<mod>(m - 1), n));
            for (int k = 1; k <= j; ++k) {
                dp[i][j] += dp[i - 1][j - k] * nCr[n - (j - k)][k] * pow(modnum<mod>(m), j - k) * pow(modnum<mod>(m - 1), n - j);
            }
        }
    }
    cout << dp[n][n] << "\n";
