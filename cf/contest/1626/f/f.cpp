...
const int maxk = 20, maxl = 8e5;
modnum<mod> dp[maxk][maxl];

void test_case() {
    int n;
    long long a_0, x, y, k, M;
    cin >> n >> a_0 >> x >> y >> k >> M;
    ...
    modnum<mod> mult = modnum<mod>(k) * pow(modnum<mod>(n), k - 1), sol = 0;
    for (int i = 0; i < n; ++i) {
        sol += mult * l * (a[i] / l);
        ++dp[1][a[i] % l];
    }
    for (int i = 1; i <= k; ++i) {
        for (int j = 0; j < l; ++j) {
            sol += j * dp[i][j] * pow(modnum<mod>(n), k - i);
            if (i < k) {
                dp[i + 1][j] += dp[i][j] * (n - 1);
                dp[i + 1][j - j % i] += dp[i][j];
            }
