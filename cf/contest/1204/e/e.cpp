...
const int inf = 0x3f3f3f3f, mod = 998244853, maxn = 4e3 + 5;
const long long INFL = 0x3f3f3f3f3f3f3f3f;
modnum<mod> nCr[maxn][maxn], dp[maxn][maxn], dp2[maxn][maxn];
...
void test_case() {
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            dp2[i][j] = nCr[i + j - 1][j] + dp2[i - 1][j];
            dp2[i][j] += dp2[i][j - 1] - nCr[i + j - 1][j - 1] + dp[i][j - 1];
        }
    }
    cout << dp2[n][m] << "\n";
}
